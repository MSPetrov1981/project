...
from db import execute_query, connect_to_db
from utils import configure_treeview_style

class EmployeeDBApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Управление базой данных сотрудников")
        ...
        self.conn = connect_to_db(self.db_params)
        if not self.conn:
            self.root.destroy()
            return
    ...
    # Все остальные методы класса EmployeeDBApp без изменений
    # (create_widgets, load_employees, sort_treeview, 
    #  apply_filter, apply_sort, show_hierarchy, 
